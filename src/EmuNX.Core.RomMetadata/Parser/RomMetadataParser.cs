...
/// configure some parameters (like <c>ConfigLanguageName</c>), then you Read the fields. Each Read methods
...
{
    // Result
    public Types.RomMetadata RomMetadata { get; private set; } = new();
    // Configuration
    ...
    private string? _controlNcaIdHex;
    private IFileSystem? _controlNcaFs;
    private UniqueRef<IFile> _controlNcaFile;
    // Nacp: Name
    ...
    {
        // We need a LEGAL keyset before this works.
        if (!CanLoadRootFsFromRom()) return RomMetadataParserError.RomReadDependenciesNotComplete;
        DisposeRootFs();

        // Try to open the XCI Secure partition which stores multiple NCAs
    ...
    private RomMetadataParserError? LoadRootFsFromNsp(string nspPath)
    {
        ...
        // Try to open the PFS0 which stores multiple NCAs
    ...
    {
        // We need RootFS opened before loading
    ...
    /// <summary>
    /// Loads the Control NCA which stores <c>control.nacp</c> and icon.
    /// </summary>
    /// <returns>RomMetadataParserError if an error occurs, otherwise null.</returns>
    public RomMetadataParserError? LoadControlNca()
    {
        // We need to have loaded CNMT
    ...
    /// <summary>
    /// Loads <c>control.nacp</c> which stores the ROM name and other data.
    /// </summary>
    /// <returns>RomMetadataParserError if an error occurs, otherwise null.</returns>
    public RomMetadataParserError? LoadNacp()
    {
        // We need to have loaded the CONTROL FileSystem
    ...
    {
        RomMetadata.Name = _nacp.Title[ConfigLanguageName.GetNameTranslationIndex()].NameString.ToString();
    ...
    private void DisposeRootFs()
    {
        ...
        DisposeCnmt();
        // New metadata
        RomMetadata = new Types.RomMetadata();
        // Free memory
        _rootFs?.Dispose();
